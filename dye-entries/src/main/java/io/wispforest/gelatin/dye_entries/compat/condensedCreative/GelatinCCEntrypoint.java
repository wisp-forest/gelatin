...
public class GelatinCCEntrypoint implements CondensedCreativeInitializer {
    ...
    public void onInitializeCondensedEntries(boolean refreshed) {
        ...
        DyeableItemVariant.getAllItemVariants().stream().filter(dyeableBlockVariant -> !dyeableBlockVariant.alwaysReadOnly()).forEach(dyeableItemVariant -> {
            CondensedEntryRegistry.fromItemTag(dyeableItemVariant.variantIdentifier, dyeableItemVariant.getDefaultItem(), dyeableItemVariant.getPrimaryItemTag())
                    .setTitleString(Text.translatable(dyeableItemVariant.variantIdentifier.getPath() + "_condensed"))
                    .setEntrySorting(allStacks -> sortItemStacks(allStacks, dyeableItemVariant.getDefaultItem(), stack -> ((DyeItemStorage)stack.getItem()).getDyeColorant()))
