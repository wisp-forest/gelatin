package io.wispforest.jello.main.common.compat.consistencyplus.data.providers;
...
public class ConsistencyPlusTagProvider {
    ...
    public static class BlockTagProvider extends FabricTagProvider.BlockTagProvider {
        ...
        private static final Logger LOGGER = LogManager.getLogger(ConsistencyPlusTagProvider.BlockTagProvider.class);
...
//            });
        ...
        private void createAndFillTag(TagKey<Block> identified){
            ...
            if(blockNamePrefix != null){
                defaultBlock = Registry.BLOCK.get(new Identifier(minecraftNameSpace ? "minecraft" : ConsistencyPlusTags.CONSISTENCY_PLUS_MODID, blockNamePrefix + "_" + blockNameSuffix));
                LOGGER.info("[1]:" +  new Identifier(minecraftNameSpace ? "minecraft" : ConsistencyPlusTags.CONSISTENCY_PLUS_MODID, blockNamePrefix + "_" + blockNameSuffix));
            }else{
                defaultBlock = Registry.BLOCK.get(new Identifier(minecraftNameSpace ? "minecraft" : ConsistencyPlusTags.CONSISTENCY_PLUS_MODID, blockNameSuffix));
                LOGGER.info("[2]:" + new Identifier(minecraftNameSpace ? "minecraft" : ConsistencyPlusTags.CONSISTENCY_PLUS_MODID, blockNameSuffix));
            }
            ...
            for(int i = 0; i < DYE_VALUES.length; i++){
                ...
                if(blockNamePrefix != null){
                    this.getOrCreateTagBuilder(identified).add(Registry.BLOCK.get(new Identifier(ConsistencyPlusTags.CONSISTENCY_PLUS_MODID, blockNamePrefix + "_" + DYE_VALUES[i].getName() + "_" + blockNameSuffix)));
                }else{
                    this.getOrCreateTagBuilder(identified).add(Registry.BLOCK.get(new Identifier(isItBlueIce ? "minecraft" : ConsistencyPlusTags.CONSISTENCY_PLUS_MODID, DYE_VALUES[i].getName() + "_" + blockNameSuffix)));
                }
            }
