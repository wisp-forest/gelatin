...
import com.dragon.jello.common.Util.MessageUtil;
...
public class RandomDyeColorStuff {
    ...
    public static DyeColorRegistry.DyeColor getRandomlyRegisteredDyeColor(){
        boolean nonVanillaDyeColor = false;
        DyeColorRegistry.DyeColor dyeColor = DyeColorRegistry.DYE_COLOR.getRandom(new Random());

        while(!nonVanillaDyeColor){
            if(!DyeColorRegistry.VANILLA_DYES.contains(dyeColor)){
                nonVanillaDyeColor = true;
            }else{
                dyeColor = DyeColorRegistry.DYE_COLOR.getRandom(new Random());
            }
        }
